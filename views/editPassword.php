...
<body>
    ...
    <main>
        <div class="container" style="border: 2px solid var(--border-color);
    padding: 2rem;
    border-radius: 1rem;
    max-width: 50rem;
    margin: auto;">
            <form id="updatePassword" class="contact__form" onsubmit="updateMemberPassword(event)">
                ...
                <div class="update__buttons">
                    <div class="button">
                        <input class="successBtn" type="submit" value="Update">
                    </div>
                </div>
            </form>
    ...
    <script>
        function updateMemberPassword(event) {
            event.preventDefault();

            const formData = new FormData();
            formData.append('memberID', <?php echo $memberID; ?>);
            formData.append('password', document.getElementById('newPassword').value);
            formData.append('confirmPassword', document.getElementById('confirmNewPassword').value);
            fetch('http://localhost/Projects/TODF/index.php?action=updateMemberPassword', {
                    method: 'POST',
                    body: formData,
                })
